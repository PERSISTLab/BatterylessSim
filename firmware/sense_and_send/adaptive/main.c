...
int main(void)
{
  ...
  while (1) {
    ...
    if (count >= send_rate)
    {
      ...
      send_rate = rate_from_voltage(read_voltage_adc());
...
uint16_t rate_from_voltage(int Volts)
{
	// Voltage = 4 + ((double) ADC12MEM0 / 32)
  double voltage = 100 * (4 + ((double) Volts / 32));
  siren_command("%s %i!\n", "PRINTF: Voltage:", (int) voltage);
	return 10;
}
